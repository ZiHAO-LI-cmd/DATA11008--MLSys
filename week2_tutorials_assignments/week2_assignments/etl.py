...
def batch_creator(df: pd.DataFrame, context: FileDataContext, data_source_name: str) -> BatchRequest:
    ...
    try:
        ...
        datasource = context.sources.add_pandas(name=data_source_name)
...
def etl(path: Path, 
    ...
    data_checkpoint_result = create_checkpoint(context=context,
                                                batch_request=batch_request,
                                                checkpoint_name=gx_checkpoint_name,
                                                expectation_suite_name=gx_expectation_suite_name,
                                                run_name=gx_run_name)
    print(1)
    context.view_validation_result(data_checkpoint_result)
